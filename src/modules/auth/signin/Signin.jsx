...
export default function Signin() {
  ...
  const { email, password, showPassword, loading, msgError, disableBtn } =
    state;
  ...
  const checkValidForm = () => {
    ...
    if (!validateEmail(email)) {
      errors.email = t("auth:wrong_format_email");
    }
  ...
  return (
    <form onSubmit={handleSignIn} className="auth">
      <div className="auth-header">
        <h3 className="title-label">{t("auth:welcome_back")}</h3>
        <p className="txt-sm txt-gray mt-1">{t("auth:login_description")}</p>
      </div>
      <div className="auth-content">
        <div className="flex-column gap-6">
          <p className="txt-sm">{t("auth:email")}</p>
          <div>
        ...
        <div className="flex-column gap-6">
          <p className="txt-sm">{t("auth:password")}</p>
          <div>
            <div className="relative">
              <input
        ...
        <button
          className={classNames(
            "btn-default w-100 btn-main fs-3 h-8 justify-center",
